...
//===----------------------------------------------------------------------===//
//
// Collect instructions that should be output at the module level and prepare
// the global register numbering.
//
// The results of this analysis are used in AsmPrinter to rename registers
// globally and to output required instructions at module level.
//
//===----------------------------------------------------------------------===//
...
InstrList &ModuleAnalysisInfo::getMSInstrs(unsigned MSType) {
  return MS[MSType];
}
