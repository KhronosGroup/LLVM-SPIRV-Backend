  ...
  for (auto &MBB : MF) {
    for (auto &MI : MBB) {
      if (MI.getOpcode() == TargetOpcode::G_INTRINSIC_W_SIDE_EFFECTS &&
          (MI.getIntrinsicID() == Intrinsic::spv_store ||
           MI.getIntrinsicID() == Intrinsic::spv_load)) {
        auto MOp = MI.getOperand(3);
        auto *ConstMI = MRI.getVRegDef(MOp.getReg());
        assert(ConstMI->getOpcode() == TargetOpcode::G_CONSTANT);
        MI.RemoveOperand(3);
        MI.addOperand(
            MachineOperand::CreateImm(ConstMI->getOperand(1).getCImm()->getZExtValue()));
      }
    }
  }
}
