...
bool SPIRVInstructionSelector::select(MachineInstr &I) {
  ...
  SPIRVType *ResType = HasDefs ? GOR.getSPIRVTypeForVReg(ResVReg) : nullptr;
...
bool SPIRVInstructionSelector::spvSelect(Register ResVReg,
  ...
  case TargetOpcode::G_SHUFFLE_VECTOR: {
    ...
    auto MIB = BuildMI(BB, I, I.getDebugLoc(), TII.get(SPIRV::OpVectorShuffle))
                   .addDef(ResVReg)
                   .addUse(GOR.getSPIRVTypeID(ResType))
  ...
  case TargetOpcode::G_PTR_ADD: {
    ...
    Register Idx =
        buildZerosVal(GOR.getOrCreateSPIRVIntegerType(32, I, TII), I);
    MachineBasicBlock &BB = *I.getParent();
...
bool SPIRVInstructionSelector::selectMemOperation(Register ResVReg,
  ...
  if (I.getOpcode() == TargetOpcode::G_MEMSET) {
    ...
    SPIRVType *ValTy = GOR.getOrCreateSPIRVIntegerType(8, I, TII);
    SPIRVType *ArrTy = GOR.getOrCreateSPIRVArrayType(ValTy, Num, I, TII);
    Register Const = GOR.getOrCreateConsIntArray(Val, I, ArrTy, TII);
    SPIRVType *VarTy = GOR.getOrCreateSPIRVPointerType(
    ...
    Type *LLVMArrTy = ArrayType::get(
        IntegerType::get(GOR.CurMF->getFunction().getContext(), 8), Num);
    GlobalVariable *GV =
...
bool SPIRVInstructionSelector::selectAtomicRMW(Register ResVReg,
  ...
  // auto ScSem =
  // getMemSemanticsForStorageClass(GOR.getPointerStorageClass(Ptr));
...
bool SPIRVInstructionSelector::selectAtomicCmpXchg(Register ResVReg,
  ...
  SPIRVType *SpvValTy = GOR.getSPIRVTypeForVReg(Val);
  ...
  bool Result =
      BuildMI(*I.getParent(), I, DL, TII.get(SPIRV::OpAtomicCompareExchange))
          .addDef(ACmpRes)
          .addUse(GOR.getSPIRVTypeID(SpvValTy))
          .addUse(Ptr)
          .addUse(ScopeReg)
          .addUse(MemSemEqReg)
          .addUse(MemSemNeqReg)
  ...
  SPIRVType *BoolTy = GOR.getOrCreateSPIRVBoolType(I, TII);
  Result |= BuildMI(*I.getParent(), I, DL, TII.get(SPIRV::OpIEqual))
                .addDef(CmpSuccReg)
                .addUse(GOR.getSPIRVTypeID(BoolTy))
                .addUse(ACmpRes)
                .addUse(Cmp)
                .constrainAllUses(TII, TRI, RBI);
  ...
  Result |= BuildMI(*I.getParent(), I, DL, TII.get(SPIRV::OpCompositeInsert))
                ...
                .addUse(GOR.getSPIRVTypeID(ResType))
                .addUse(ACmpRes)
                .addUse(GOR.getOrCreateUndef(I, ResType, TII))
                .addImm(0)
                .constrainAllUses(TII, TRI, RBI);
  Result |= BuildMI(*I.getParent(), I, DL, TII.get(SPIRV::OpCompositeInsert))
                .addDef(ResVReg)
...
bool SPIRVInstructionSelector::selectAddrSpaceCast(Register ResVReg,
  ...
  if (!UIs.empty() && ++UIs.begin() == UIs.end() &&
    ...
    MachineBasicBlock &BB = *I.getParent();
    SPIRVType *SpvBaseTy = GOR.getOrCreateSPIRVIntegerType(8, I, TII);
    ResType = GOR.getOrCreateSPIRVPointerType(SpvBaseTy, I, TII,
  ...
  SPIRVType *SrcPtrTy = GOR.getSPIRVTypeForVReg(SrcPtr);
  SPIRV::StorageClass::StorageClass SrcSC = GOR.getPointerStorageClass(SrcPtr);
  SPIRV::StorageClass::StorageClass DstSC = GOR.getPointerStorageClass(ResVReg);
...
bool SPIRVInstructionSelector::selectCmp(Register ResVReg,
  ...
  assert(GOR.getSPIRVTypeForVReg(Cmp0)->getOpcode() ==
             GOR.getSPIRVTypeForVReg(Cmp1)->getOpcode() &&
...
bool SPIRVInstructionSelector::selectICmp(Register ResVReg,
  ...
  if (GOR.isScalarOfType(CmpOperand, SPIRV::OpTypePointer))
    CmpOpc = getPtrCmpOpcode(Pred);
  else if (GOR.isScalarOrVectorOfType(CmpOperand, SPIRV::OpTypeBool))
...
SPIRVInstructionSelector::buildI32Constant(uint32_t Val, MachineInstr &I,
  ...
  Type *LLVMTy = IntegerType::get(GOR.CurMF->getFunction().getContext(), 32);
  const SPIRVType *SpvI32Ty =
      ResType ? ResType : GOR.getOrCreateSPIRVIntegerType(32, I, TII);
  // Find a constant in DT or build a new one.
  auto ConstInt = ConstantInt::get(LLVMTy, Val);
  Register NewReg = GOR.find(ConstInt, GOR.CurMF);
  if (!NewReg.isValid()) {
    NewReg = MRI->createGenericVirtualRegister(LLT::scalar(32));
    GOR.add(ConstInt, GOR.CurMF, NewReg);
    MachineInstr *MI;
    MachineBasicBlock &BB = *I.getParent();
    if (Val == 0) {
      MI = BuildMI(BB, I, I.getDebugLoc(), TII.get(SPIRV::OpConstantNull))
               .addDef(NewReg)
               .addUse(GOR.getSPIRVTypeID(SpvI32Ty));
    } else {
...
Register SPIRVInstructionSelector::buildZerosVal(const SPIRVType *ResType,
  ...
  if (ResType->getOpcode() == SPIRV::OpTypeVector)
    return GOR.getOrCreateConsIntVector(0, I, ResType, TII);
  return GOR.getOrCreateConstInt(0, I, ResType, TII);
}

Register SPIRVInstructionSelector::buildOnesVal(bool AllOnes,
                                                const SPIRVType *ResType,
                                                MachineInstr &I) const {
  unsigned BitWidth = GOR.getScalarOrVectorBitWidth(ResType);
  APInt One =
      AllOnes ? APInt::getAllOnes(BitWidth) : APInt::getOneBitSet(BitWidth, 0);
  if (ResType->getOpcode() == SPIRV::OpTypeVector)
    return GOR.getOrCreateConsIntVector(One.getZExtValue(), I, ResType, TII);
  return GOR.getOrCreateConstInt(One.getZExtValue(), I, ResType, TII);
}
...
bool SPIRVInstructionSelector::selectSelect(Register ResVReg,
  ...
  bool IsScalarBool =
      GOR.isScalarOfType(I.getOperand(1).getReg(), SPIRV::OpTypeBool);
  unsigned Opcode =
...
bool SPIRVInstructionSelector::selectIToF(Register ResVReg,
  ...
  if (GOR.isScalarOrVectorOfType(I.getOperand(1).getReg(), SPIRV::OpTypeBool)) {
    unsigned BitWidth = GOR.getScalarOrVectorBitWidth(ResType);
    SPIRVType *TmpType = GOR.getOrCreateSPIRVIntegerType(BitWidth, I, TII);
    if (ResType->getOpcode() == SPIRV::OpTypeVector) {
      const unsigned NumElts = ResType->getOperand(2).getImm();
      TmpType = GOR.getOrCreateSPIRVVectorType(TmpType, NumElts, I, TII);
    }
...
bool SPIRVInstructionSelector::selectTrunc(Register ResVReg,
                                           const SPIRVType *ResType,
                                           MachineInstr &I) const {
  if (GOR.isScalarOrVectorOfType(ResVReg, SPIRV::OpTypeBool)) {
    Register IntReg = I.getOperand(1).getReg();
    const SPIRVType *ArgType = GOR.getSPIRVTypeForVReg(IntReg);
    return selectIntToBool(IntReg, ResVReg, I, ArgType, ResType);
  }
  bool IsSigned = GOR.isScalarOrVectorSigned(ResType);
  unsigned Opcode = IsSigned ? SPIRV::OpSConvert : SPIRV::OpUConvert;
  return selectUnOp(ResVReg, ResType, I, Opcode);
}
...
bool SPIRVInstructionSelector::selectConst(Register ResVReg,
  ...
  if (TyOpcode == SPIRV::OpTypeInt) {
    ...
    Register Reg = GOR.getOrCreateConstInt(Imm.getZExtValue(), I, ResType, TII);
    if (Reg == ResVReg)
      return true;
...
bool SPIRVInstructionSelector::selectGlobalValue(
  ...
  SPIRVType *ResType = GOR.getOrCreateSPIRVType(
  ...
  if (isa<Function>(GV)) {
    ...
    MachineBasicBlock &BB = *I.getParent();
    Register NewReg = GOR.find(ConstVal, GOR.CurMF);
    if (!NewReg.isValid()) {
      SPIRVType *SpvBaseTy = GOR.getOrCreateSPIRVIntegerType(8, I, TII);
      ResType = GOR.getOrCreateSPIRVPointerType(SpvBaseTy, I, TII);
      Register NewReg = ResVReg;
      GOR.add(ConstVal, GOR.CurMF, NewReg);
      return BuildMI(BB, I, I.getDebugLoc(), TII.get(SPIRV::OpConstantNull))
          .addDef(NewReg)
          .addUse(GOR.getSPIRVTypeID(ResType))
          .constrainAllUses(TII, TRI, RBI);
  ...
  Register Reg = GOR.buildGlobalVariable(ResVReg, ResType, GlobalIdent, GV,
