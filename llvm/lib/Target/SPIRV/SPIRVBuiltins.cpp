...
static bool generateSpecConstantInst(const IncomingCall *Call,
  ...
  unsigned SpecId = static_cast<unsigned>(getIConstVal(Call->Arguments[0], MRI));
  buildOpDecorate(Call->ReturnRegister, MIRBuilder, Decoration::SpecId, {SpecId});
  ...
  assert(Const &&
         ...
         "Argument should be either int or floating-point constant");

  // Determine the opcode and built the OpSpec MI.
  unsigned Opcode = OpSpecConstant;
