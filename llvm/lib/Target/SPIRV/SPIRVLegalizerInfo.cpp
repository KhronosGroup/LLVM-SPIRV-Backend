...
static Register convertPtrToInt(Register Reg, LLT ConvTy, SPIRVType *SpirvType,
  ...
  GTR->assignSPIRVTypeToVReg(SpirvType, ConvReg, Helper.MIRBuilder.getMF());
...
bool SPIRVLegalizerInfo::legalizeCustom(LegalizerHelper &Helper,
  ...
  if (!isTypeFoldingSupported(Opc)) {
    ...
    assert(GTR->getSPIRVTypeForVReg(MI.getOperand(0).getReg()));
    ...
    if ((!ST->canDirectlyComparePointers() ||
      ...
      Type *LLVMTy = IntegerType::get(MI.getMF()->getFunction().getContext(),
                                      ST->getPointerSize());
      SPIRVType *SpirvTy = GTR->getOrCreateSPIRVType(LLVMTy, Helper.MIRBuilder);
      Op0.setReg(convertPtrToInt(Reg0, ConvT, SpirvTy, Helper, MRI, GTR));
      Op1.setReg(convertPtrToInt(Reg1, ConvT, SpirvTy, Helper, MRI, GTR));
    }
    return true;
  }
