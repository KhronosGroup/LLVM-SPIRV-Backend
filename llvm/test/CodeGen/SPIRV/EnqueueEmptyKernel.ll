...
; CHECK-SPIRV: OpName %[[Block:[0-9]+]] "__block_literal_global"
; CHECK-SPIRV: %[[Int8:[0-9]+]] = OpTypeInt 8
; CHECK-SPIRV: %[[Void:[0-9]+]] = OpTypeVoid
; CHECK-SPIRV: %[[Int8Ptr:[0-9]+]] = OpTypePointer CrossWorkgroup %[[Int8]]
; CHECK-SPIRV: %[[Int8PtrGen:[0-9]+]] = OpTypePointer Generic %[[Int8]]
; CHECK-SPIRV: %[[Block:[0-9]+]] = OpVariable %{{[0-9]+}}
...
; CHECK-SPIRV: %{{[0-9]+}} = OpEnqueueKernel %{{[0-9]+}} %{{[0-9]+}} %{{[0-9]+}} %{{[0-9]+}} %{{[0-9]+}} %{{[0-9]+}} %{{[0-9]+}} %[[Invoke:[0-9]+]] %[[Int8PtrGenBlock]] %{{[0-9]+}} %{{[0-9]+}}
}
...
; CHECK-SPIRV: %[[Invoke]] = OpFunction %[[Void]] {{.*}} %{{[0-9]+}}
; CHECK-SPIRV-NEXT: %{{[0-9]+}} = OpFunctionParameter %[[Int8PtrGen]]
