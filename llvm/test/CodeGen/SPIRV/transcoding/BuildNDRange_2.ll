...
; CHECK-SPIRV-DAG:     %[[ARRAY_T2:[0-9]+]] = OpTypeArray %{{[0-9]+}} %[[LEN2_ID]]
; CHECK-SPIRV-DAG:     %[[ARRAY_T3:[0-9]+]] = OpTypeArray %{{[0-9]+}} %[[LEN3_ID]]

; CHECK-SPIRV-LABEL:   OpFunction
; CHECK-SPIRV:         %[[LOAD2_ID:[0-9]+]] = OpLoad %[[ARRAY_T2]]
; CHECK-SPIRV:         %{{[0-9]+}} = OpBuildNDRange %{{[0-9]+}} %[[LOAD2_ID]]
; CHECK-SPIRV:         %[[LOAD3_ID:[0-9]+]] = OpLoad %[[ARRAY_T3]]
; CHECK-SPIRV:         %{{[0-9]+}} = OpBuildNDRange %{{[0-9]+}} %[[LOAD3_ID]]
; CHECK-SPIRV-LABEL:   OpFunctionEnd

; CHECK-SPIRV-LABEL:   OpFunction
; CHECK-SPIRV:         %[[CONST_LOAD2_ID:[0-9]+]] = OpLoad %[[ARRAY_T2]]
; CHECK-SPIRV:         %{{[0-9]+}} = OpBuildNDRange %{{[0-9]+}} %[[CONST_LOAD2_ID]]
; CHECK-SPIRV:         %[[CONST_LOAD3_ID:[0-9]+]] = OpLoad %[[ARRAY_T3]]
; CHECK-SPIRV:         %{{[0-9]+}} = OpBuildNDRange %{{[0-9]+}} %[[CONST_LOAD3_ID]]
; CHECK-SPIRV-LABEL:   OpFunctionEnd
