...
; CHECK-SPIRV: %[[ArrayTypeID:[0-9]+]] = OpTypeImage %{{[0-9]+}} 1D 0 1 0 0 Unknown ReadOnly
...
; CHECK-SPIRV: %[[ArrayVarID:[0-9]+]] = OpFunctionParameter %[[ArrayTypeID]]
; CHECK-SPIRV: %{{[0-9]+}} = OpImageQuerySizeLod %{{[0-9]+}} %[[ArrayVarID]]
; CHECK-SPIRV-NOT: %{{[0-9]*}} = OpExtInst %{{[0-9]*}} %{{[0-9]*}} get_image_array_size
