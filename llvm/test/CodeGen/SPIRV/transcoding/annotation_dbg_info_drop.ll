; RUN: llc -O0 %s -o - | FileCheck %s --check-prefixes=CHECK,CHECK-SPV
...
; CHECK: OpName %[[ANNO_PUMP_ID:[0-9]+]] ".str"
; CHECK: OpName %[[FILE_ID:[0-9]+]] ".str.1"
; CHECK: OpName %[[FPGA_REG_ID:[0-9]+]] ".str.2"
; CHECK: OpName %[[ANNO_NUMBANKS_ID:[0-9]+]] ".str.3"
; CHECK: OpName %[[FUNC_ID:[0-9]+]] "_ZZZ4mainENK3$_0clERN2cl4sycl7handlerEENKUlvE_clEv"
; CHECK: OpName %[[S_E_STRUCT_ID:[0-9]+]] "struct._ZTSZZZ4mainENK3$_0clERN2cl4sycl7handlerEENKUlvE_clEvE3s_e.s_e"
; CHECK: OpName %[[F_ID:[0-9]+]] "_Z1fv"

; CHECK: %[[S_E_STRUCT_PTR_ID:[0-9]+]] = OpTypePointer {{.*}} %[[S_E_STRUCT_ID]]
...
; CHECK:      %{{[0-9]+}} = OpExtInst %{{[0-9]+}} %{{[0-9]+}} DebugNoScope
...
; CHECK:      %{{[0-9]+}} = OpExtInst %{{[0-9]+}} %{{[0-9]+}} DebugNoScope
; CHECK-NEXT: %{{[0-9]+}} = OpInBoundsPtrAccessChain %{{[0-9]+}} %[[ANNO_PUMP_ID]]
; CHECK-NEXT: %{{[0-9]+}} = OpInBoundsPtrAccessChain %{{[0-9]+}} %[[FILE_ID]]
  call spir_func void @_Z1fv(), !dbg !74
; -- var.annotation call is dropped. Restore debug scope after the call.
; CHECK-NEXT: %{{[0-9]+}} = OpExtInst %{{[0-9]+}} %{{[0-9]+}} DebugScope
; CHECK-NEXT: OpLine
; CHECK-NEXT: %{{[0-9]+}} = OpFunctionCall %{{[0-9]+}} %[[F_ID]]
...
; CHECK:      %{{[0-9]+}} = OpExtInst %{{[0-9]+}} %{{[0-9]+}} DebugNoScope
; CHECK-NEXT: %{{[0-9]+}} = OpInBoundsPtrAccessChain %{{[0-9]+}} %[[FPGA_REG_ID]]
...
; CHECK:      %{{[0-9]+}} = OpExtInst %{{[0-9]+}} %{{[0-9]+}} DebugNoScope
; CHECK-NEXT: %{{[0-9]+}} = OpInBoundsPtrAccessChain %{{[0-9]+}} %[[ANNO_NUMBANKS_ID]]
...
; CHECK:      %{{[0-9]+}} = OpBitcast %{{[0-9]+}} %[[AGG_TMP_ID]]
